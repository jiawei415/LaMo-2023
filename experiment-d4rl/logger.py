...
def init_wandb(config) -> None:
    ...
    group = f"{config.group}-{config.env}-{corruption_info}"
    project = "CorruptionOffline"
    wandb_dir = os.path.expanduser(os.path.join(config.outdir, config.group))
    wandb.init(
...
def init_logger(args):
    ...
    args.logfile = f"{args.alg_type}_{env_name}_{args.seed}_{time_tag}"
    log_path = os.path.join(args.outdir, args.group, env_name, args.logfile)
